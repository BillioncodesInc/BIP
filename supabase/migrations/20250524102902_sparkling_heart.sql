/*
  ...
  1. Enums
    - User roles (root, admin, editor)
    - Portfolio categories (web, mobile, design, ecommerce, software)
  ...
  2. Tables
    ...
    - Portfolio Items (project showcase)
    - Blog Posts (news and articles)
    - Events (workshops, bootcamps, etc.)
  ...
  3. Security
    ...
    - Policies for data access and modification
*/
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_team_members_order') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_portfolio_items_slug') THEN
    CREATE INDEX idx_portfolio_items_slug ON portfolio_items(slug);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_portfolio_items_order') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_programs_status') THEN
    CREATE INDEX idx_programs_status ON programs(status);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_store_products_slug') THEN
    CREATE INDEX idx_store_products_slug ON store_products(slug);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_store_products_status') THEN
    CREATE INDEX idx_store_products_status ON store_products(status);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_store_products_category') THEN
    CREATE INDEX idx_store_products_category ON store_products(category);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_donations_created_at') THEN
    CREATE INDEX idx_donations_created_at ON donations(created_at);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_volunteers_status') THEN
    CREATE INDEX idx_volunteers_status ON volunteers(status);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_faq_items_category') THEN
    CREATE INDEX idx_faq_items_category ON faq_items(category);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_faq_items_order') THEN
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_store_products_updated_at') THEN
