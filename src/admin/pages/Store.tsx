...
const Store = () => {
  ...
  const columns = [
    ...
    columnHelper.accessor('price', {
      ...
      cell: (info) => (
        <div>
          {info.row.original.sale_price ? (
            <>
              <span className="text-gray-500 line-through">${info.getValue()}</span>
              <span className="ml-2 text-green-600">${info.row.original.sale_price}</span>
            </>
          ) : (
    ...
    columnHelper.accessor('downloads', {
      ...
      cell: (info) => (
        <div>
          <div className="font-medium">{info.getValue()} downloads</div>
          <div className="flex items-center">
            <div className="flex text-yellow-400">
              {'★'.repeat(Math.floor(info.row.original.rating))}
              {'☆'.repeat(5 - Math.floor(info.row.original.rating))}
            </div>
            <span className="ml-2 text-sm text-gray-500">
              ({info.row.original.review_count})
            </span>
          </div>
        </div>
      ),
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600">Manage digital products and courses</p>
        </div>
